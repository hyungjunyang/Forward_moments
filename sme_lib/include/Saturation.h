...
void GaussianQuadrature(int qPoint, double* &rloc, double* &weight);// Pipatl or quadrature.cc
...
public:
  ...
  void solveMT(double* timeArray, int tSize, int* swStrl, int endPointFlag); // calculate saturation moments with multiple time
  void output(int i);									// output

  double* calcSatCovEndPoints();	// calc sat cov among end points of all streamlines (Pipatl)
  double getSatAvg(int ii){return satAvg[ii];}//Pipatl
  double getSatVar(int ii){return satVar[ii];}//Pipatl
...
private:
  ...
  int nLn, nNd, nPoint;
  double *tauMn, *tauVar;
  double *rhoLntau; //Need to calculate Css (Pipatl)
  double *x1C, *etaMnC;
  ...
  // --- Variables for new conditioning saturatin data structure (Pipatl)
  double* timeArray;
  int tSize;
  int* swStrl;
  int endPointFlag;
  ...
  double calcSatCovQuad(double time1, double time2, double satAvg1, double satAvg2, double tauMn1, double tauMn2, double tauVar1, double tauVar2, double rhoLntau12);
  ...
  ////These functions are replaced by the new functions with quadrature integration
  //void calcSatAvgVar();                               // calc sat mean,vari
  //void calcSatAvgVar2();								// calc sat mean,vari by dlntau(Pipatl)
  //void calcSatAvgVarQuad();							// calc sat mean, vari by quadrature intregation(Pipatl)
  //double calcSatCov(double satAvg1, double satAvg2, double tauMn1, double tauMn2, double tauVar1, double tauVar2, double rhoLntau12);
  //// calc sat covariance by integration over bivariate pdf for point 1 and 2 (Pipatl)
  //double calcSatCov2(double satAvg1, double satAvg2, double tauMn1, double tauMn2, double tauVar1, double tauVar2, double rhoLntau12);
