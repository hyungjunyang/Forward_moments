...
void PCondiInverse::addMem_of_readData() {
 ...
 meas_err_mat = new double[num_p_Meas * num_p_Meas];
}
...
void PCondiInverse::readData() {
   ...
   //in_Pcondi >> num_p_Meas >> meas_err_opt;
   if(debug) cout<<"num_p_Meas = " << num_p_Meas <<endl;
   if(debug) cout<<"meas_err_opt = " << meas_err_opt <<endl;
   ...
   for(int m = 0; m < num_p_Meas; ++m) {
       ...
       istream.str(iline);
       istream.clear();
       istream >> i_p_Meas[m] >> j_p_Meas[m] >> k_p_Meas[m]
                 >> p_avg_Meas[m];
       if (meas_err_opt>0) {
          for(int ierr = 0; ierr <= m; ++ierr) {
            istream >> meas_err_mat[m*num_p_Meas+ierr];
	    meas_err_mat[ierr*num_p_Meas+m] = meas_err_mat[m*num_p_Meas+ierr];
          }
       }
       --i_p_Meas[m];
       --j_p_Meas[m];
   ...
   if(debug && (meas_err_opt > 0)) {
     cout << endl << "Meas Error Matrix:" << endl;
     for(int ierr1 = 0; ierr1 < num_p_Meas; ++ierr1) {
       for(int ierr2 = 0; ierr2 < num_p_Meas; ++ierr2) {
         cout << meas_err_mat[ierr1*num_p_Meas+ierr2] << " ";
       }
       cout << endl;
     }
     cout << endl;
...
void PCondiInverse::solve(int nWells, const Well *wellArr, const Control &c, 
   ...
   for(int ijk2 = 0; ijk2 < gridPtr->getNumPermNode(); ++ijk2) {
       for(int ijk1 = 0; ijk1 < gridPtr->getNumPermNode(); ++ijk1) {
	       ...
	       if(meas_err_opt == 2){
		  for(int msu_j2 = 0; msu_j2 < num_p_Meas; ++msu_j2) {
		      //setCYY_Cond below will subtract the value off the original
		      value -= mu[msu_j + ijk1 * num_p_Meas]
			       * meas_err_mat[msu_j + (msu_j2 * num_p_Meas)]
			       * mu[msu_j2 + ijk2 * num_p_Meas];
		  }
	       }
           }
   ...
   // [Pipatl] This shouldn't be needed, the Cond CPP is calculated directly from perm field
...
void PCondiInverse::calcMu(double *aa, double *aaInv, double *mu) {
   for(int msu_k = 0; msu_k < num_p_Meas; ++msu_k) {
	   ...
	   if(meas_err_opt == 1) {
	      aa[msu_j + msu_k * num_p_Meas] += meas_err_mat[msu_j + msu_k * num_p_Meas];
	   }
       }
   }
