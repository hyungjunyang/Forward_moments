...
void SwCondiInverse::ReadData()
{
	...
	Junk(in_SwCondi);
	dFactor = -9999;
	in_SwCondi >> condScheme >> dFactor; //dFactor remains -9999 if there is no input
	if(dFactor==-9999)
	{	
		cerr<<"Input of damping factor is required in trans_condi.in"<<endl;
		exit(8);
	}
	Junk(in_SwCondi);
	...
	{
		Junk(in_SwCondi);
		in_SwCondi >> condTime[icond] >> coordTemp.i_ >> coordTemp.j_ >> coordTemp.k_ >> swMeas[icond];
...
void SwCondiInverse::SequentialConditioning(char *directory, ifstream &is_flow)
{
	...
	while(true)
	{
		...
		while(true)
		{
			...
			if(((MaxAbs(numSw,satErr)<=etol)&&(MaxAbs(numSw,satVarEP)<=vtol))||(iter==maxIter))
			{
			  if(nObv>0){
			    delete[] pointPtr;
			    delete bTrackPtr;
...
void SwCondiInverse::GlobalConditioning(char *directory, ifstream &is_flow)
{
	...
	while(true)
	{
		...
		if((nObv>0)&&(iter==0))
		  {CalcSaturationMoments(obvSet,0,nObv,oof,iter,0);}
		//Check whether the conditioning is convert
		if(((MaxAbs(nCond,satErr)<=etol)&&(MaxAbs(nCond,satVarEP)<=vtol))||(iter==maxIter))
		{
		  if(nObv>0){
		    delete[] pointPtr;
		    delete bTrackPtr;
