...
void Particle::CoordTrans() {
   ...
   double vxo,vyo;//(Pipat) Use to calculate dv_deta
   ...
   for(int i = 0; i < length; ++i) {
       ...
       //dv_deta[i] = (-dvx + dvy) * sinth[i] * costh[i]; //This calculate interface value, but dv_deta is cell value
       if(i == 0) {
          dv_deta[0] = 0.;
       } else {  
          x_po = trajectory[i - 1].getX();
          y_po = trajectory[i - 1].getY();
	  if(x_p!=x_po) dvx = (vx-vxo)/(x_p-x_po);
	  if(y_p!=y_po) dvy = (vy-vyo)/(y_p-y_po);
          dv_deta[i] = (-dvx+dvy)*(0.5*(sinth[i-1]+sinth[i]))*(0.5*(costh[i-1]+costh[i]));
	  /*
	  double vxmid = 0.5*(vx+vxo);
	  double vymid = 0.5*(vy+vyo);
	  double vavgmid = sqrt(vxmid*vxmid+vymid*vymid);
	  dv_deta[i] = (-dvx+dvy)*(vymid/vavgmid)*(vxmid/vavgmid);
	  */
       }
       vxo = vx;
       vyo = vy;
   ...
   for(int i = 1; i < trajectory.size(); ++i) {
       ...
       tmp2   = 1./ v_avg[i-1] 
		 ...
		 //- etadu[i-1] //The correct derivation shouldn't have this term (Pipat)
		    + Eta_Eta[(i - 1) * (1+ length)] / v_avg[i-1]/v_avg[i-1] * dv_deta[i] * dv_deta[i]
		    + 2. * Vxi_Eta[(i - 1) *(1+length)] * dv_deta[i] / v_avg[i-1] / v_avg[i-1]
                );                       
       dt     = 0.5 * (tmp1 + tmp2) * ds[i];
       if(dt<0) dt = 0.5*((1./v_avg[i])+(1./v_avg[i-1]))*ds[i];
       t_sum += dt;
       tau_Avg2Trans[i] = t_sum;
       //cout<<tau_Avg2Trans[i]<<" "<<Vxi_Eta[(i-1)*(1+length)]<<" "
       //	   <<Vxi_Eta[i*(1+length)]<<" "<<dv_deta[i]<<endl;
   }
   //cout<<endl;
...
void Particle::calcTauVar(){
   ...
   // Due to the complication that dv_deta is cell-center, not interface
   // f[] will be calculated at the time of integration
   /*
   for(int l2 = 0; l2 < length; l2++) {
   ...
   */
   ...
   for(int l2 = 1; l2 < length; l2++) {
	...
	for(int pp=0;pp<4;pp++){
          l1l2 = (l1+l1m[pp]) + (l2+l2m[pp]) * length;
	  l2l1 = (l2+l2m[pp]) + (l1+l1m[pp]) * length;
	  f_sum += ( Vxi_Vxi[l1l2]
		     + Vxi_Eta[l1l2] * dv_deta[l2]
		     + Vxi_Eta[l2l1] * dv_deta[l1]
		     + Eta_Eta[l1l2] * dv_deta[l1] * dv_deta[l2]
		     ) /pow(v_avg[l1+l1m[pp]],2.) /pow(v_avg[l2+l2m[pp]],2.);
	}
	/*
          tmp += (  f[l1l2 - 1] + f[l1l2 - length]
                  + f[l1l2] + f[l1l2 - 1 - length]
                  ...
                  * ds[l1] * ds[l2] * 2.0;
	*/
	tmp += f_sum/4.0*ds[l1]*ds[l2]*2.0;
      }
      f_sum = 0;
      for(int pp=0;pp<4;pp++){
	l2l2 = (l2+l1m[pp]) + (l2+l2m[pp]) * length;
	l2l1 = (l2+l2m[pp]) + (l2+l1m[pp]) * length;
	f_sum += ( Vxi_Vxi[l2l2]
		   + Vxi_Eta[l2l2] * dv_deta[l2]
		   + Vxi_Eta[l2l1] * dv_deta[l2]
		   + Eta_Eta[l2l2] * dv_deta[l2] * dv_deta[l2]
		   ) /pow(v_avg[l2+l1m[pp]],2.) /pow(v_avg[l2+l2m[pp]],2.);
      }
      /*
      l2l2 = l2 + l2 * length;    
      tmp += (  f[l2l2] + f[l2l2 - 1] + f[l2l2 - 1] 
